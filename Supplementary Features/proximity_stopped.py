...
newpath = 'Top_100_stopped_docs_using_proximity_encoded/'
if not os.path.exists(newpath):
    os.makedirs(newpath)

with open("stopped_inverted_list_unigram_position_encoded.txt", 'rb') as f:
...
f=open('Top_100_stopped_pages_for_queries_using_proximity.txt', 'w')
for query in query_dict.values():
    ...
    for id in final_score1:
           if (c <= 100):
               ...
               c += 1
    output = open(newpath + 'Top_100_stopped_documents_using_proximity_for_query' + '_%d' %i + '_encoded.txt', 'wb')
