...
f = open('TF_IDF_Top100_Pages.txt', 'w')
for query in query_list:
    ...
    for quid in final_score1:
        ...
        c += 1
    newpath = r'../../Encoded Data Structures/Encoded-TF-IDF-Top100Docs-perQuery/'
    if not os.path.exists(newpath):
        os.makedirs(newpath)
    output = open(
        newpath + 'Encoded-Top100Docs-TF-IDF' + '_%d' % i + '.txt', 'wb')
...
list_output = open('TF_IDF_Top5_Docs.txt', 'w')
...
output = open('TF_IDF_Top5_Query_Pages.txt', 'w')
output.write(str(top_5))
output.close()
encoded_output = open('../../Encoded Data Structures/Encoded-TF_IDF_Top5_Query_Pages.txt', 'wb')
