...
path = r'C:\Users\virat\PycharmProjects\Project\Task 3\Part B\Stemmed_Corpus'
...
output = open('inverted_list_unigram_Stemming.txt', 'w+', encoding = 'utf-8')
for term in inv_list_unigram:
    output.write("%s -> %s\n" %(term , inv_list_unigram[term]))
output.close()

output = open('inverted_list_unigram_Stemming_encoded.txt' , 'wb')
pickle.dump(inv_list_unigram , output)
output.close()

f = open("DocumentID_DocLen_Stemming.txt", 'w', encoding = 'utf-8')
f.write(str(docID_doclen))
f.close()

output = open('DocumentID_DocLen_Stemming_encoded.txt' , 'wb')
