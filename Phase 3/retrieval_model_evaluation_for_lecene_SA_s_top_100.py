...
with open("QueryID_Top100Docs_by_lucene_SA_encoded.txt", 'rb') as f:
...
for qID in queryID_relevantDocs:
    ...
    if docName_R_N == {}:
        continue
    f = open("Precision_Recall_Tables/Lucene_StandardAnalyzer/Precision_Recall_Table_for_" +qID+'.txt', 'w')
    f.write("For Query: %s\n\n" %queryID_query[qID])
...
f1 = open("Precision_Recall_Tables/Lucene_StandardAnalyzer/Final Evaluation.txt", 'w')
...
f2 = open("Precision_Recall_Tables/Lucene_StandardAnalyzer/P@KValuesForAllQueries.txt",'w')
for qID in queryID_relevantDocs:
