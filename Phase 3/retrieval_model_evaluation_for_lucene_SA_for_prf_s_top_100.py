...
with open("QueryID_Top100Docs_by_lucene_SA_WithPRF_encoded.txt", 'rb') as f:
...
def calc_R_N_list(q):
    ...
    if q in queryID_relevantDocs:
        ...
        top_100_docs = queryID_top100Docs[int(q)]
...
for qID in queryID_relevantDocs:
    ...
    if docName_R_N == {}:
        continue
    f = open("Precision_Recall_Tables/Lucene_StandardAnalyzer_WithPRF/Precision_Recall_Table_for_" +qID+'.txt', 'w')
    f.write("For Query: %s\n\n" %queryID_query[qID])
...
f1 = open("Precision_Recall_Tables/Lucene_StandardAnalyzer_WithPRF/Final Evaluation.txt", 'w')
...
f2 = open("Precision_Recall_Tables/Lucene_StandardAnalyzer_WithPRF/P@KValuesForAllQueries.txt",'w')
for qID in queryID_relevantDocs:
